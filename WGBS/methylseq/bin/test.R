...
import_data <- function(files, logfile) {
    ...
    for (f in files) {
        if (file.info(f)$size > 0) {
            name <- gsub("_ecoli_qc\\.bed", "", basename(f))
...
create_plot <- function(lam_files, out_files, log_file) {
    cat("Loading data\n", file=log_file)
...
create_plot(args, "out.pdf", "out.log")
